...
> Tips2：  
> 下载的ROM包路径： `%USERPROFILE%\Documents\HiSuite\ROM` 并回车  
> 实际路径为：C(系统盘):\Users\用户名\Documents(文档)\HiSuite\ROM  

保留的ROM包不动，通过数据线连接需要升降级的设备，打开`华为手机助手`的`系统升级`界面，此时应会自动检测到存放的ROM包，应该可以执行升降级操作  

> Tips：关于 eRecovery 模式
> 1.通过 `音量+` + `电源键` 进入 `eRecovery模式`   
> 2.不同华为设备进入 `eRecovery模式` 的方法不同，请根据实际情况操作！  
